...
```{r}
complete.cases(head(df)) #Nest functions 
```
...
```{r}
sum(complete.cases(head(df))) #Could then save this as an object to use the number later
```
...
not_NA #To see true for the values that are not NA
sum(!is.na(df$Dried_mass_g))
```
...
```{r}
df_remove_NA_Mass <- df[complete.cases(df[,2]), ]
df_remove_NA_Mass2 <- df[complete.cases(df$Dried_mass_g), ] #Returns the same as the indexing method above

View(df_remove_NA_Mass) #Get 45 observations
```
...
Therefore our statistical method is: *one way ANOVA - b/c treatment has 3 levels*
...
```{r}
anova(grow.mod) #Answers first question of is there an overall effect of treatment on growth rate
```
...
```{r}
summary(grow.mod) #use to compare the treatment levels with the control. Would need Tukey HSD to compare treatments 1 and 2.
```
